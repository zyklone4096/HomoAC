...
@SkidClass(plugin = "XinAntiCheat", clazz = "dev.zenhao.xin.utils.CSpeed")
public class CSpeed {
    ...
    public static boolean onBlock(Location loc) {
        ...
        for (int x = xMin; x <= xMax; ++x) {
            for (int z = zMin; z <= zMax; ++z) {
                if (loc.add(x, 0.0, z).getBlock().getType() == Material.WATER_LILY) {
                    return true;
                }
                if (loc.add(x, -0.5, z).getBlock().getType() != Material.AIR && !loc.add(x, -0.5, z).getBlock().isLiquid()) {
                    return true;
                }
                Material beneath = loc.add(x, -1.5, z).getBlock().getType();
